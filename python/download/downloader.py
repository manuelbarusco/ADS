...
* crete the json file with all the dataset meta-data included the download info (number of urls dowloaded and urls total number)
  in the dataset.json file it will remove all the special characters
...
@param error_log_file file with all the errors that are thrown during the download
@param resume_row id of the dataset in the datasets.json file from which start in case of resume
'''
def readAndDownload(datasets_json_path, datasets_folder_path,  error_log_file, resume_row):
    ...
    for dataset in data['datasets']:
        ...
        if(row >= resume_row):
            ...
            json_dict_object = json.dumps(dataset_json, indent=4, ensure_ascii=False)

            #writing the json file
            with open(dataset_directory_path+"/dataset.json", "w", encoding="utf-8") as outfile:
